...
var invyy = 0; repeat(ds_list_size(player_inventory)){
	...
	var draw_row = ds_grid_value_y(library_items, 0, 0, 0, ds_grid_height(library_items), draw_item);
	var draw_display = library_items[# item_col.displayName, draw_row];
	var draw_icon = library_items[# item_col.sprite, draw_row];
...
var shopyy = 0; repeat (shop_inventory_length) {
	...
	var draw_row = ds_grid_value_y(library_items, 0, 0, 0, ds_grid_height(library_items), draw_item); // Match each value to their corresponding row in library_items
	var draw_display = library_items[# item_col.displayName, draw_row]; // Assign the Display Name column value from draw_row
	var draw_icon = library_items[# item_col.sprite, draw_row]; // Assign the Sprite column value for draw_row
...
var selected_row = ds_grid_value_y(library_items, 0, 0, 0, ds_grid_height(library_items), selected_item_id); // Locate the row in librar_items for the selected item
var draw_description = library_items[# item_col.description, selected_row]; // Assign the Item Description for the selected item from the library
